...
with check (
  ...
  and (
    ...
    or public.can_use_pro_features()
  )
);
...
begin
  ...
  if current_plan = 'free' and (
    ...
    or profile_background_type = 'video'
...
comment on function public.save_profile_editor(
  text, text, text, text, text, text, text, text, text, text, text, jsonb
) is 'Guarda imágenes de fondo en Free y mantiene los videos protegidos por Pro.';
